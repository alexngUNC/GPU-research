...
//#define TIMESLICE 2.131*1000000 // 2.13ms
//#define EPSILON 0.035*1000000 // time difference from timeslice end
#define MAX_PAGES 2014656 // total 4 KB pages for RTX 3060 Ti
#define PAGE_SIZE 4096
#define COUNT 50

__global__ void loop_on_gpu(char *page, int *flag) {
	for (int i=0; i<COUNT; i++) {
		// dereference to page fault
		*page = 0;
		// go to the next page
		page += PAGE_SIZE;
	}
...
int main(int argc, char **argv) {
	...
	if (num_iters * COUNT > MAX_PAGES) {
		fprintf(stderr, "Not enough pages to fulfill interval count. Aborting...\n");
		return 1;
	}
	CUdeviceptr d_pages;
	SAFE_D(cuMemAllocManaged(&d_pages, num_iters * PAGE_SIZE * COUNT, CU_MEM_ATTACH_GLOBAL));
	...
	*flag = 1;
	...
	for (size_t i=0; i<num_iters; i++) {
		...
		SAFE_D(cuMemAdvise(d_pages + i * PAGE_SIZE * COUNT, PAGE_SIZE * COUNT, CU_MEM_ADVISE_SET_PREFERRED_LOCATION, CU_DEVICE_CPU));
		*(((char *) d_pages) + i * PAGE_SIZE) =  1;
		 
		// Advise the pages to be on the GPU so they migrate
		SAFE_D(cuMemAdvise(d_pages + i * PAGE_SIZE * COUNT, PAGE_SIZE * COUNT, CU_MEM_ADVISE_SET_PREFERRED_LOCATION, device));

		// launch page fault kernel
		loop_on_gpu<<<1,1>>>((char *) d_pages + i * PAGE_SIZE * COUNT, flag);
