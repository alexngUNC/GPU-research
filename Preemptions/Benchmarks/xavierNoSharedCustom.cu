...
__global__ void saxpyShared(int n, float a, float *x, float *y) {
  ...
  const int ARRAY_SIZE = (int) 196608.0 * PERCENTAGE_SHARED;
  float z[ARRAY_SIZE];
...
int main() {
  ...
  int carveout = 0;
  SAFE(cudaFuncSetAttribute(saxpyShared, cudaFuncAttributePreferredSharedMemoryCarveout, carveout));

  // Execute the kernel - 2 TBs per SM for all SMs
  // 49152 bytes of shared memory per block 
  saxpyShared<<<16, 1024>>>(n, a, d_x, d_y);
